(function() {
  ...
  function calculateColors() {
    let primaryColor = window
      ...
      .trim();
    let secondaryColor = window
      .getComputedStyle(document.documentElement)
      .getPropertyValue("--mdc-theme-secondary")
      .trim();
    let secondaryColor700 = window
      .getComputedStyle(document.documentElement)
      .getPropertyValue("--mdc-theme-secondary-700")
      .trim();
  ...
  function setupDashboardCharts() {
    ...
    const walletPriceContainers = document.querySelectorAll(
      ".crypto-wallet__market"
    );
    const walletStatsContainers = document.querySelectorAll(
      ".crypto-wallet__stats"
    );
    const accountBalanceContainer = document.querySelector(
      ".crypto-wallet-overview__balance"
    );

    const coinCurrentPrices = [0, 0, 0];
    const coinWallets = [0.81215, 7.19, 32.73];
    ...
    function updateWallets() {
      ...
      walletStatsContainers.forEach(function(container, index) {
        const stat = coinInitialPrices[index]
          ? ((coinCurrentPrices[index] - coinInitialPrices[index]) /
              coinInitialPrices[index]) *
            100
          : 0.0;
        container.innerText = stat.toFixed(2) + "%";

        const statsClass =
          stat >= 0 ? "crypto-wallet__stats--up" : "crypto-wallet__stats--down";
        container.classList.remove(
          "crypto-wallet__stats--up",
          "crypto-wallet__stats--down"
        );
    ...
    function updateAccountBalance() {
      const balance =
        coinCurrentPrices[0] * coinWallets[0] +
    ...
    const coinChartLinks = document.querySelectorAll(
      ".crypto-widget__actions a"
    );
    coinChartActions.addEventListener(
      "click",
      function(e) {
        ...
        activeCoinIndex = Array.prototype.findIndex.call(coinChartLinks, link =>
          link.classList.contains("mdc-button--raised")
        );
        e.preventDefault();
      },
      false
  ...
  const tableExpandTogglesContainer = document.querySelector(
    ".crypto-widget__content .mdl-data-table"
  );
  tableExpandTogglesContainer.addEventListener("click", function(e) {
    ...
    while (element !== e.currentTarget) {
      if (element.classList.contains("crypto-transactions-list__item-toggle")) {
        ...
        element.parentNode.parentNode.nextElementSibling.classList.toggle(
          "expanded"
        );
        e.preventDefault();
        return;
  ...
  // Wallet
  const newWalletDialog = new mdc.dialog.MDCDialog(
    document.querySelector(".mdc-dialog")
  );
  document
    .querySelector(".crypto-wallet--new")
