(function() {
  ...
  function setupDashboardCharts() {
    ...
    document
      .querySelector(".data-monthly-refresh")
    ...
    document
      .querySelector(".data-yearly-refresh")
      .addEventListener("click", function(e) {
        clearInterval(timedInterval);
        chart.updateOptions(
          {
    ...
    document
      .querySelector(".data-weekly-refresh")
      .addEventListener("click", function(e) {
        clearInterval(timedInterval);
  ...
  const transactionTabBar = new mdc.tabBar.MDCTabBar(
    document.querySelector("#crypto-wallet-transaction")
  );
  ...
  function cryptoUpdateTabPanels(panels, index) {
    const activePanel = panels.querySelector(
      ".crypto-wallet-form-panel.crypto-wallet-form-panel--active"
    );
    if (activePanel) {
      activePanel.classList.remove("crypto-wallet-form-panel--active");
    }
    const newActivePanel = panels.querySelector(
      ".crypto-wallet-form-panel:nth-child(" + (index + 1) + ")"
    );
    if (newActivePanel) {
  ...
  document
    ...
    .forEach(text => new mdc.textField.MDCTextField(text));
